...
IslayPub.system_dialog = new function(){
	...
	this.open=function(){
		/*
		window.removeEventListener('touchmove', function(event) {//スクロール無効
		...
		*/
		...
		var d = IslayPub.dialog.open(
			...
			"<div>"+
				...
				"<select id='system_dialog_background_color' value='#ffffff'>"+
	...
	this.apply = function(){
		...
		alert(document.getElementById("system_dialog_background_color").value);
		...
		if(document.getElementById("system_background_color").value != document.getElementById("system_dialog_background_color").value){
			...
			alert("aaa_num" + num);
			document.getElementById("system_background_color").value = num;
		}
