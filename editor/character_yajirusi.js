	...
	*/
	...
	this.create = function(maru,event,init){//矢印つくるときはinitはundefined //修正した
      ...
      svg.setAttribute('ontouchstart', 'IslayPub.character.yajirusi.touch_start(this,event)');
			...
			alert(from_maru+":"+to_maru);

			svg.setAttribute('transition_condition', "none");
			svg.setAttribute('add_time' , 0);
			//svg.setAttribute('transition_condition1', "none");//増やした
			//svg.setAttribute('transition_condition2', "none");//増やした
			...
			if(end_id !== maru_info.id){//一回目と二回目のタッチしたじょうたいが異なる場合のみ
				...
				if(maru_svg2.length > 0){//矢印がないときは処理を行わない
					for(var i = 0; i < maru_svg2.length; ++i){
						...
						//console.log("from_maru:" + from_maru + ":to_maru:" + to_maru);
...
*/
	...
	*/
	this.move = function(yajirusi, type, event, point){//修正した
		...
		//var x = event.clientX-rect.left;//こっちじゃないとiOSで矢印が表示されない event変数受け渡したらできるかな？
		//var y = event.clientY-rect.top;//こっちじゃないとiOSで矢印が表示されない
		...
		//console.log("end.x:" + end.x +"tp1.x" + tp1.x);
		//計算結果を適用
